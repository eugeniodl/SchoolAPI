    ...
    public class AttendanceController : ControllerBase
    {
        ...
        public async Task<ActionResult<AttendanceDto>> GetAttendance(int id)
        {
            if (id <= 0)
            {
                ...
                return BadRequest("ID de aistencia no válido");
            }
        ...
        public async Task<ActionResult<AttendanceDto>> PostAttendance(AttendanceCreateDto createDto)
        {
            ...
            try
            {
                _logger.LogInformation($"Creando una nueva asistencia para el estudiante " +
                    $"con ID: {createDto.StudentId} en la fecha: {createDto.Date}");

                // Verificar si el estudiante existe
                var studentExists = await _studentRepo.ExistsAsync(
                    s => s.StudentId == createDto.StudentId);
            ...
            AttendanceUpdateDto updateDto)
        {
            if (updateDto == null || id != updateDto.AttendanceId)
            {
                return BadRequest("Los datos de entrada no son válidos o " +
                    "el ID de asistencia no coincide.");
            }
            ...
            try
            {
                ...
                var existingAttendance = await _attendanceRepo.GetById(id);
                if (existingAttendance == null)
                {
                    _logger.LogInformation($"No se encontró ninguna asistencia con ID: {id}");
                    return NotFound("La asistencia no existe.");
                }

                // Verificar si el estudiante existe
                var studentExists = await _studentRepo.ExistsAsync(
                    s => s.StudentId == updateDto.StudentId);
            ...
            {
                _logger.LogError($"Error al actualizar la asistencia: {ex.Message}");
                return StatusCode(StatusCodes.Status500InternalServerError,
                    "Error interno del servidor al actualizar la asistencia.");
            }
        }
