...
namespace School_API.Controllers
{
    ...
    public class AttendanceController : ControllerBase
    {
        ...
        public AttendanceController(IStudentRepository studentRepo,
            ...
            ILogger<AttendanceController> logger, IMapper mapper)
        {
        ...
        public async Task<ActionResult<AttendanceDto>> GetAttendance(int id)
        {
            if (id <= 0)
            {
                _logger.LogError($"ID de asistencia no válida: {id}");
                return BadRequest("ID de asistencia no válida");
            }
        ...
        public async Task<ActionResult<AttendanceDto>> PostAttendance(AttendanceCreateDto createDto)
        {
            ...
            try
            {
                _logger.LogInformation($"Creando un nueva asistencia para el estudiante con " +
                    $"ID: {createDto.StudentId} en la fecha: {createDto.Date}");

                // Verificar si el estudiante existe
                var studentExists = await _studentRepo
                    .ExistsAsync(s => s.StudentId == createDto.StudentId);
                ...
                // Verificar si la asistencia ya existe para la fecha y el estudiante
                var existingAttendance = await _attendanceRepo
                    .GetAsync(a => a.StudentId == createDto.StudentId 
                    && a.Date == createDto.Date);

                if (existingAttendance != null)
                {
                    _logger.LogWarning($"La asistencia para el estudiante con ID " +
                        $"'{createDto.StudentId}' ya existe para la fecha '{createDto.Date}'");
                    ModelState.AddModelError("AsistenciaExiste", 
                        "¡La asistencia para esa fecha ya existe!");
                    return BadRequest(ModelState);
                }
                ...
                return CreatedAtAction(nameof(GetAttendance), 
                    new { id = newAttendance.AttendanceId }, newAttendance);
            }
            catch (Exception ex)
            {
        ...
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> PutAttendance(int id, 
            AttendanceUpdateDto updateDto)
        {
            if(updateDto == null || id != updateDto.AttendanceId)
            {
                return BadRequest("Los datos de entrada no son válidos " +
                    "o el ID de la asistencia no coincide.");
            }
            ...
            try
            {
                ...
                // Verificar si el estudiante existe
                var studentExists = await _studentRepo
                    .ExistsAsync(s => s.StudentId == updateDto.StudentId);
                ...
                // Actualizar solo las propiedades necesarias de la asistencia existente
            ...
            {
                if(!await _attendanceRepo.ExistsAsync(a => a.AttendanceId == id))
                {
                    _logger.LogWarning($"No se encontró ninguna asistencia con ID: {id}");
            ...
            {
                _logger.LogError($"Error al actualizar la asistencia con ID {id}: " +
                    $"{ex.Message}");
                return StatusCode(StatusCodes.Status500InternalServerError,
                    "Error interno del servidor al actualizar la asistencia.");
            }
        ...
        public async Task<IActionResult> DeleteAttendance(int id)
        {
            ...
            {
                _logger.LogError($"Error al eliminar la asistencia con ID {id}: " +
                    $"{ex.Message}");
                return StatusCode(StatusCodes.Status500InternalServerError,
